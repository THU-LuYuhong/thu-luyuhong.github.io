...
def testeqsupport():
  supported = True
  msg = ''
  p = Popen('latex --version', shell=True, stdout=PIPE, stderr=PIPE, universal_newlines=True)
  ...
  if rc != 0:
    ...
    first = out.splitlines()[0].rstrip() if out else ''
    msg += '  latex: ' + first + '.\n'
  p = Popen('dvipng --version', shell=True, stdout=PIPE, stderr=PIPE, universal_newlines=True)
  out, err = p.communicate()
  rc = p.returncode
  if rc != 0:
...
class controlstruct(object):
  ...
  def pushfile(self, newfile):
    ...
    self.inf = open(newfile, 'r', encoding='utf-8', errors='replace')
...
def parseconf(cns):
  ...
  for sname in cns:
    fs.append(open(sname, 'r', encoding='utf-8', errors='replace'))
...
def insertmenuitems(f, mname, current, prefix):
  m = open(mname, 'r', encoding='utf-8', errors='replace')
...
def pc(f, ditchcomments=True):
  ...
  if c: # only undo forward movement if we're not at the end.
    ...
    # move back one character using absolute position (safer for text files)
    try:
      f.inf.seek(f.inf.tell() - 1)
    except (OSError, IOError):
      # fallback to relative seek if supported
      f.inf.seek(-1, 1)
  elif f.otherfiles:
    f.nextfile()
...
def doincludes(f, l):
  ...
  i = 'include{'
  if l.startswith(ir):
    nf = open(l[len(ir):-2], 'r', encoding='utf-8', errors='replace')
...
def geneq(f, eq, dpi, wl, outname):
  ...
  if f.eqcache:
    try:
      dc = open(os.path.join(f.eqdir, '.eqdepthcache'), 'r', encoding='utf-8', errors='replace')
      for l in dc:
        a = l.split()
  ...
  try:
    ...
    p = Popen(latexcmd, shell=True, stdout=PIPE, stderr=PIPE, universal_newlines=True)
    out, err = p.communicate()
    rc = p.returncode
    if rc != 0:
    ...
    p = Popen(dvicmd, shell=True, stdout=PIPE, stderr=PIPE, universal_newlines=True)
    out, err = p.communicate()
    rc = p.returncode
    if rc != 0:
      print(err)
      raise Exception('dvipng error')
    depth = int(out.splitlines()[-1].split('=')[-1])
  finally:
    # Clean up.
    for ext in exts:
      g = basefile + ext
  ...
  if f.eqcache and eqname not in eqdepths:
    try:
      dc = open(os.path.join(f.eqdir, '.eqdepthcache'), 'a', encoding='utf-8', errors='replace')
...
def codeblock(f, g):
  ...
  if raw:
    ...
    output,_ = Popen(ext_prog, shell=True, stdin=PIPE,
                     stdout=PIPE, universal_newlines=True).communicate(buff)
    out(f.outf, output)
  else:
    if g[1] == 'jemdoc':
...
def main():
  ...
  for inname in innames:
    ...
    infile = open(inname, 'r', encoding='utf-8', errors='replace', newline='')
